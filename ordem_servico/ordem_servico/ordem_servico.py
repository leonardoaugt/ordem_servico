...
def get_tempo_orcamento(equipamento):
    ...
    return frappe.db.sql("SELECT tempo_orcamento FROM `tabFamilias de Equipamentos` WHERE name = (SELECT familia FROM tabEquipamentos WHERE name=%s" % (equipamento), as_dict=True)